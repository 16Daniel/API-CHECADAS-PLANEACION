...
namespace API_PEDIDOS.Controllers
{
        ...
        public async Task<ActionResult> GetPedidos(int idu)
        {
            try
            {
              ...
              int countday = numdia;
                            ...
                            var artinvsem = _dbpContext.CheckInvSemanals.Where(x => x.Codarticulo == art.cod).FirstOrDefault();
                            double inventario = 0;
                            ...
                            {
                                inventario = await _fp.obtenerInvnetarioArtSemanal(item.Codsucursal, art.cod);
                                if (inventario != -999999){ hayinventario = true; } else { hayinventario = false; status = 2; }
                            }
                        ...
                        if (requierecartones) 
                        {
                            ...
                            if (inventarioteorico)
                            {
                                ...
                                invcartones = await _fp.getInventarioTeorico(item.Codsucursal, 10277); 
                            }
                            else
                            {
                                invcartones = await _fp.getInventario(item.Codsucursal, 10277);
                            }
    ...
    [HttpGet]
    [Route("GetMedidasUdsSucursales")]
    public async Task<ActionResult> getumedidasSucursales()
    {
      try
      {

        var proveedores = _dbpContext.PedSucProveedores
                    ...
                    foreach (var articulo in articulos)
          {
            ...
            if (umedidaart != null)
            {
              medidas.Add(new umedidaModel()
              {
                ...
                umedida = umedidaart.Umedida,
                uds = umedidaart.Uds,
                nomarticulo = artdb.Descripcion,
                nomprov = provdb.Nomproveedor
              });
            }
